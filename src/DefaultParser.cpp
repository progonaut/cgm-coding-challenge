...
InputParser::ParserOutput DefaultParser::parse( std::string line ) {
	...
	for( auto character = line.begin( ); character != line.end( ) && !error; ) {
		switch( step ) {
					  ...
					  { PTT::Error,
					    util::buildString(
					      ...
					      *character,
					      "' at index ",
					      std::distance( line.begin( ), character ),
						  " when parsing question.",
					      "Expected character or end of question (?), not begin of answer definition." ) } );
					error = true;
				} else {
			...
			case QuestionDone: {
				if( isws(*character)) {
					// skip white-space after question was read
				} else if( *character == '"' ) {
			...
			case AnswerDone: {
				if( isws(*character)) {
					// skip whitespace between answers
				} else if( *character == '"' ) {
					step = Answer;
					pos_marker = character+1;
				} else {
	...
	// QuestionDone
	// Init
	while( output.size() != 0 && output.back().size() == 0 ) output.pop_back();
	if( error ) {
		if( output.size() > 1 ) {
		...
		if( output.size() == 0 ) newStatement();
			if( step == Question ) {
				output.back().push_front({PTT::Error, util::buildString("Unexpected end-of-input when parsing question. Expected '?'.")});
			} else if( step == Answer ) {
				output.back().push_front({PTT::Error, util::buildString("Unexpected end-of-input when parsing answer. Expected '\"'.")});
			} else  {
				output.back().push_front({PTT::Error, util::buildString("Unexpected end-of-input when parsing in step ", std::size_t(step), ".")});
			}
			if( output.size() > 1 ) {
				output.push_front({ output.back().front() });
			}
	...
	return std::accumulate(std::next(output.begin()), output.end(), *output.begin(), 
